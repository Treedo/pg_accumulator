...
-- ================================================================
\echo 'Scenario 1/8  register_post() single inserts (500,000 ops)...'

DO $$
DECLARE
    t_start timestamptz;
    t_end   timestamptz;
    N       constant int := 500000;
    i       int;
BEGIN
    t_start := clock_timestamp();
    FOR i IN 1..N LOOP
        PERFORM accum.register_post('b_std_100', jsonb_build_object(
            'recorder', 'bench:s1:' || i,
            'period',   '2026-01-15',
            'warehouse', (i % 10) + 1,
            'product',   (i % 50) + 1,
...
-- ================================================================
\echo 'Scenario 2/8  register_post() batch 10 (50000 × 10 = 500,000 items)...'

DO $$
DECLARE
    t_start    timestamptz;
    t_end      timestamptz;
    N_batches  constant int := 50000;
...
BEGIN
    ...
    FOR i IN 1..N_batches LOOP
        ...
        PERFORM accum.register_post('b_std_100', batch);
    END LOOP;
    t_end := clock_timestamp();
...
-- ================================================================
\echo 'Scenario 3/8  register_post() batch 100 (10000 × 100 = 1,000,000 items)...'

DO $$
DECLARE
    t_start   timestamptz;
    t_end     timestamptz;
    N_batches constant int := 10000;
...
-- ================================================================
\echo 'Scenario 4/8  register_post() batch 1000 (1000 × 1000 = 1,000,000 items)...'
...
-- ================================================================
\echo 'Scenario 5/8  balance_cache direct read (200,000 point lookups)...'
...
BEGIN
    ...
    FOR i IN 1..N LOOP
        ...
        FROM accum.b_std_100_balance_cache
...
-- ================================================================
-- SCENARIO 6: b_std_100_balance() per-register function
-- ================================================================
\echo 'Scenario 6/8  b_std_100_balance() per-register function (100,000 calls)...'

DO $$
DECLARE
    t_start timestamptz;
    t_end   timestamptz;
    N       constant int := 100000;
    i       int;
    dummy   jsonb;
BEGIN
    t_start := clock_timestamp();
    FOR i IN 1..N LOOP
        SELECT accum.b_std_100_balance(
    ...
    END LOOP;
    t_end := clock_timestamp();
    PERFORM pg_temp.bench_record('6. b_std_100_balance() per-register fn', N, t_start, t_end);
END;
$$;
...
-- ================================================================
\echo 'Scenario 7/8  register_post() high_write delta buffer (500,000 ops)...'
...
BEGIN
    t_start := clock_timestamp();
    FOR i IN 1..N LOOP
        PERFORM accum.register_post('b_hw_100', jsonb_build_object(
            'recorder', 'bench:s7:' || i,
            'period',   '2026-01-15',
            'warehouse', (i % 10) + 1,
            'product',   (i % 50) + 1,
...
-- ================================================================
\echo 'Scenario 8/8  register_unpost() (100,000 cancellations)...'
...
BEGIN
    ...
    t_start := clock_timestamp();
    FOR i IN 1..N LOOP
        PERFORM accum.register_unpost('b_std_100', 'bench:s8:' || i);
    END LOOP;
    t_end := clock_timestamp();
